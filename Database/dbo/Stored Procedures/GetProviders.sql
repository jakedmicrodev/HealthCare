...
-- Description:	Get provider(s)
-- EXEC [dbo].[GetProviders] null
-- =============================================
...
BEGIN
	...
	SELECT
		...
		,p.[LastName]
		,p.[BusinessName]
		,IIF(p.[BusinessName] IS NULL, p.[FirstName] + IIF(p.[MI] = '', ' ', ' ' + p.[MI] + ' ') + p.[LastName], p.[BusinessName]) AS [Name]
		,p.[TitleId]
		,t.[Name] AS [Title]
		,p.Address1
		,p.Address2
